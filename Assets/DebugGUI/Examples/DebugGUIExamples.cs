...
#pragma warning disable 0414
    ...
    float CosProperty { get { return Mathf.Cos(UnityEngine.Time.time * 6); } }
    ...
    float SinProperty { get { return Mathf.Sin((UnityEngine.Time.time + Mathf.PI / 2) * 6); } }
#endif
    ...
    void Update()
    {
        ...
        SinField = Mathf.Sin(UnityEngine.Time.time * 6);
        ...
        DebugGUI.LogPersistent("smoothFrameRate", "SmoothFPS: " + (1 / UnityEngine.Time.deltaTime).ToString("F3"));
        DebugGUI.LogPersistent("frameRate", "FPS: " + (1 / UnityEngine.Time.smoothDeltaTime).ToString("F3"));

        if (UnityEngine.Time.smoothDeltaTime != 0)
            DebugGUI.Graph("smoothFrameRate", 1 / UnityEngine.Time.smoothDeltaTime);
        if (UnityEngine.Time.deltaTime != 0)
            DebugGUI.Graph("frameRate", 1 / UnityEngine.Time.deltaTime);
    }

    void FixedUpdate()
    {
        // Manual graphing
        DebugGUI.Graph("fixedFrameRateSin", Mathf.Sin(UnityEngine.Time.time * 6));
    }
